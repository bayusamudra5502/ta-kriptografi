...
\begin{table}[!h]
  ...
  \begin{tabular}{|c|p{6cm}|c|}
    ...
    P9 & Ancaman \emph{flooding} kepada partisipan komunikasi & 2 \\ \hline
  \end{tabular}
\end{table}
...
\begin{table}[!h]
  ...
  \begin{tabular}{|c|p{6cm}|c|}
    ...
    S6 & Pesan terenkripsi dilindungi oleh MAC yang melibatkan nomor frame & P4 dan P9 \\ \hline
...
Permasalahan terkait dengan \emph{replay attack} pada dasarnya sudah teratasi dengan mekanisme MAC yang dinamis. Selain itu, pada protokol TLS, perhitungan nilai MAC didasari juga dengan nomor frame. Oleh karena itu, pesan sudah terlindungi dari ancaman tersebut.
...
Permasalahan terkait dengan \emph{denial of service} dapat dikurangi dampaknya dengan melakukan perhitungan MAC pada pesan yang telah terenkripsi. Pesan hanya boleh dilakukan dekripsi bila nilai MAC benar. Hal ini dapat meminimalisir dampak dari serangan \emph{flooding} yang dilakukan oleh penyerang.
...
\begin{equation}
  ...
  \begin{array}{l}
    H = \text{HMAC}(\text{key}, \text{frame\_number}\text{ }|| \\ 
      ...
      \text{   }\text{frame}.\text{encrypted\_data} \\
    ) 
  \end{array}
\end{equation}
...
Pada proses pengiriman pesan, pesan terlebih dahulu harus dipotong menjadi beberapa frame. Setiap frame pesan akan dilakukan proses \emph{padding} dengan kelipatan 16 bytes. Setelah pemasangan padding, akan dilakukan proses enkripsi. Setelah proses enkripsi berhasil, nilai MAC dari proses enkripsi tersebut akan dihitung sesuai dengan persamaan \ref{eq:tls.data.mac}.
